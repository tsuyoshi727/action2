...
let jltx = ($.getval('jltx') || '1');//提现金额
if(!$.isNode()&&jlkktxhd.indexOf("\n") ==-1){
    jlkkhdArr.push($.getdata('jlkkhd'))
    jlkktxhdArr.push($.getdata('jlkktxhd'))
} else {
    if($.isNode()){
    if (process.env.JLKKTX_HD && process.env.JLKKTX_HD.indexOf('\n') > -1) {
        jlkktxhd = process.env.JLKKTX_HD.split('\n');
    } else {
        jlkktxhd = [process.env.JLKKTX_HD]
    };
    if (process.env.JLKK_HD && process.env.JLKK_HD.indexOf('\n') > -1) {
        jlkkhd = process.env.JLKK_HD.split('\n');
    } else {
        jlkkhd = [process.env.JLKK_HD]
    };
    console.log(` ============脚本执行 - 北京时间 (UTC + 8)：${new Date(new Date().getTime() + 8 * 60 * 60 * 1000).toLocaleString()} =============\n`);
 } else if(!$.isNode()&&jlkktxhd.indexOf("\n")>-1){
   jlkktxhd = jlkktxhd.split("\n")
   jlkkhd = jlkkhd.split("\n")
};
    Object.keys(jlkktxhd).forEach((item) =>{
        if (jlkktxhd[item]) {
        jlkktxhdArr.push(jlkktxhd[item])
        }
    });
    Object.keys(jlkkhd).forEach((item) =>{
        if (jlkkhd[item]) {
            jlkkhdArr.push(jlkkhd[item])
        }
    });		
